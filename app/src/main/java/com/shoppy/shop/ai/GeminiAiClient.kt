...
class GeminiAiClient(private val apiKey: String) {
    ...
    suspend fun getProductInsights(productInfo: String, userMessage: String): String = withContext(Dispatchers.IO) {
        try {
            val prompt = """
                You are a helpful shopping assistant. Use the following product information to provide insights and answer questions:
    
                $productInfo
    
                User question: $userMessage
                
                Provide concise, accurate and helpful responses based on this product information.
            """.trimIndent()
